...
## 如何保证缓存与数据库数据的一致性
由于引入了缓存，在缓存超时时间内，可能存在数据库与缓存不匹配。  

简单的思路： 
1. 读时，有缓存读缓存，没有缓存读数据库再写缓存。
2. 写时，更新数据库，再更新缓存。

但是这个方法会有问题：
* 在写时，先写了库，在删除缓存前，写库的线程宕机了，没有删除掉缓存，会出现数据不一致情况。
...
1. 缓存失效时间变短 ：治标不治本
2. 增加cache更新重试机制 ： 如果 cache 服务当前不可用导致缓存删除失败的话，我们就隔一段时间进行重试，重试次数可以自己定。
