...
redis的近似LRU：
* LRU算法需要一个双向链表来记录数据的最近被访问顺序，但是出于节省内存的考虑，Redis的LRU算法并非完整的实现。Redis并不会选择最久未被访问的键进行回收，相反它会尝试运行一个近似LRU的算法，通过对少量键进行取样，然后回收其中的最久未被访问的键。
...
## HyperLogLog的妙用
统计一个集合中不重复元素的个数，常见于计算独立用户数

这个方法存在误差但是精度较高，而且内存使用是固定的。
...
## RDB与AOF的区别
* AOF每一条指令都会记录，而rdb只有到只有手动提交save命令，或者是关闭命令时，才触发备份操作
* 也就是说RDB粒度大，aof粒度小
* AOF不需要同步 
* AOF有重写。因为 AOF 的运作方式是不断地将命令追加到文件的末尾， 所以随着写入命令的不断增加， AOF 文件的体积也会变得越来越大
