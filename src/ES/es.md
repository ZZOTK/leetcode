...
目前优惠项目，es和数据库的数据一致性是同步写的。因为是b端系统且使用不多，同步写比较容易实现和保证一致性

订单服务中的数据同步是通过binlog实现的。数据库通过canal 到 rocketmq发送消息 给数据同步服务，进行es的同步。

使用rocketmq是他可以保证消息的稳定性和有序性。如果消息消费失败，会重试消费。数据库消息是带有version的，不用担心消费之前的消息。
